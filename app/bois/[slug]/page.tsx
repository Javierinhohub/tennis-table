...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: produit } = await supabase
    .from("produits")
    .select("id, nom, slug, marques(nom, pays, site_web), bois(nombre_plis, composition, type_jeu, type_manche, taille_lame, poids, vitesse_note, controle_note, rigidite)")
    .eq("slug", slug)
    .single()
  ...
  return (
    <main style={{ maxWidth: "1100px", margin: "0 auto", padding: "2.5rem 2rem" }}>
      <a href="/bois" style={{ color: "var(--accent)", textDecoration: "none", fontSize: "13px", fontWeight: 500, display: "inline-flex", alignItems: "center", gap: "4px", marginBottom: "1.5rem" }}>
        Retour aux bois
      </a>
