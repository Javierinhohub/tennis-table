...
// ── Composant étoiles (readonly uniquement ici) ───────────────────────────────
function Etoiles({ note, onChange, readonly = false }: { note: number; onChange?: (n: number) => void; readonly?: boolean }) {
  ...
  return (
    <div>
      ...
      {(hover > 0 || note > 0) && (
        <p style={{ fontSize: "13px", fontWeight: 600, color: COLORS[hover || note] }}>{LABELS[hover || note]}</p>
      )}
    </div>
  )
}
          ...
          <form onSubmit={handleAvisSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div>
              ...
              <input type="text" value={titre} onChange={e => setTitre(e.target.value)}
                style={inputStyle} placeholder="Ex: Excellent bois offensif" maxLength={100} />
            </div>
            <div>
      ...
      {!user && (
        <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px", textAlign: "center", marginBottom: "1.5rem" }}>
          ...
          <Link href={`/auth/login?redirect=${encodeURIComponent(pathname)}`}
            style={{ background: "#D97757", color: "#fff", textDecoration: "none", borderRadius: "8px", padding: "10px 20px", fontSize: "14px", fontWeight: 600 }}>
            Se connecter
          </Link>
        </div>
      )}
