...
export default function BoisMaterialSection({ produitId, produitNom }: { produitId: string, produitNom: string }) {
  ...
  const [membresCourants, setMembresCourants] = useState<any[]>([])  // utilisateurs.bois_id
  ...
  async function fetchData() {
    const [{ data: histoData }, { data: courantsData }] = await Promise.all([
    ...
    setMembresCourants(courantsData || [])
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
      ...
      <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: "10px", padding: "16px" }}>
        ...
        <button onClick={toggleHistorique} disabled={loading}
          ...
          {loading ? "…" : dansHistorique ? "✓ Dans mon historique" : "J'ai utilisé ce bois"}
        </button>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {(u.classement || u.club) && (
                    <p style={{ fontSize: "11px", color: "var(--text-muted)" }}>
                      {[u.classement ? u.classement + " pts" : "", u.club].filter(Boolean).join(" · ")}
                    </p>
                  )}
                </div>
