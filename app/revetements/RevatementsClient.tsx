...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const title = `${marque} ${produit.nom} — Test, avis et caractéristiques`
  const description = `Découvrez le ${marque} ${produit.nom}, revêtement ${type.toLowerCase()} de tennis de table. Vitesse ${rev?.vitesse_note || "—"}/10, Effet ${rev?.effet_note || "—"}/10, Contrôle ${rev?.controle_note || "—"}/10. Avis et notes des joueurs sur TT-Kip.`
...
export default async function RevetementPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Calcul d'un aggregateRating de fallback depuis les notes TT-Kip (/10 → /5)
  // utilisé uniquement si aucun avis utilisateur n'existe
  const ttkNotes = [rev?.vitesse_note, rev?.effet_note, rev?.controle_note].filter(Boolean)
  const ttkAvg = ttkNotes.length > 0
    ? (ttkNotes.reduce((s: number, n: number) => s + n, 0) / ttkNotes.length / 10 * 5).toFixed(1)
    : null

  // JSON-LD — aggregateRating TOUJOURS présent (avis utilisateurs ou notes TT-Kip)
  const jsonLd: Record<string, any> = {
    ...
    // offers — toujours présent (obligatoire Google)
    "offers": {
      "@type": "Offer",
      "priceCurrency": "EUR",
      "availability": "https://schema.org/InStock",
      "url": `https://www.tt-kip.com/revetements/${slug}`,
      ...(rev?.prix ? { "price": parseFloat(rev.prix) } : { "price": "0" }),
    },
    // aggregateRating — avis utilisateurs en priorité, sinon notes internes TT-Kip
    ...( avgNote ? {
      "aggregateRating": {
      ...
      // review éditorial TT-Kip en fallback
      "review": {
        "@type": "Review",
        "author": { "@type": "Organization", "name": "TT-Kip" },
        "reviewRating": {
          "@type": "Rating",
          "ratingValue": ttkAvg,
          "bestRating": "5",
        ...
        "reviewBody": `Évaluation TT-Kip du ${marque?.nom} ${produit.nom} basée sur les critères vitesse, effet et contrôle.`,
      },
    } : {
      // Dernier recours : review minimal si aucune note disponible
      "review": {
        "@type": "Review",
        "author": { "@type": "Organization", "name": "TT-Kip" },
        "reviewRating": {
        ...
        "reviewBody": `Fiche produit ${marque?.nom} ${produit.nom} sur TT-Kip, la base de données de revêtements de tennis de table.`,
      },
    }),
  }
