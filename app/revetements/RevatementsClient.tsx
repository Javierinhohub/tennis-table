...
export default function RevatementsClient({ initialProduits, initialTotal, marques }: {
  ...
  async function fetchData() {
    ...
    // Filtrer par type côté client (plus rapide que join)
    if (typeFilter) {
                ...
                >
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    {p.revetements?.type_revetement && (
                      <span style={{ fontSize: "11px", fontWeight: 600, padding: "2px 8px", borderRadius: "10px", background: p.revetements.type_revetement === "In" ? "#EBF5FF" : p.revetements.type_revetement === "Long" ? "#F0FDF4" : "#FFF7ED", color: p.revetements.type_revetement === "In" ? "#1A56DB" : p.revetements.type_revetement === "Long" ? "#0E7F4F" : "#D97757" }}>
