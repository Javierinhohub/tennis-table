...
export default function Page() {
  ...
  async function fetchData() {
    ...
    const [{ data: produits }, { data: marquesData }] = await Promise.all([
      supabase.from("produits").select("id, nom, slug, marques(id, nom), colles(type_colle, volume_ml, base_eau, temps_sechage, odeur)").eq("actif", true).order("nom").limit(2000),
  ...
  const getHref = (slug: string) => "/" + "colles/" + slug
  ...
  return (
    <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "2.5rem 2rem" }}>
      <div style={{ marginBottom: "2rem" }}>
        <h1 style={{ fontSize: "22px", fontWeight: 700, marginBottom: "4px" }}>Colles</h1>
