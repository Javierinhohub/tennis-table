...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: produit } = await supabase
    .from("produits")
    .select("id, nom, slug, marques(nom, pays, site_web), chaussures(genre, type_semelle, pointures, couleur, prix_indicatif)")
    .eq("slug", slug)
    .single()
  ...
  const infos = [
                ...
                { label: "Pointures disponibles", value: detail?.pointures },
                { label: "Couleur", value: detail?.couleur },
                { label: "Prix indicatif", value: detail?.prix_indicatif },
  ].filter(i => i.value)
