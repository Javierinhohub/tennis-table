...
export default function Page() {
  ...
  async function fetchData() {
    ...
    const [{ data: produits }, { data: marquesData }] = await Promise.all([
      supabase.from("produits").select("id, nom, slug, marques(id, nom), chaussures(genre, type_semelle, pointures, couleur, prix_indicatif)").eq("actif", true).order("nom").limit(2000),
  ...
  const getHref = (slug: string) => "/" + "chaussures/" + slug
  ...
  return (
    <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "2.5rem 2rem" }}>
      <div style={{ marginBottom: "2rem" }}>
        <h1 style={{ fontSize: "22px", fontWeight: 700, marginBottom: "4px" }}>Chaussures</h1>
